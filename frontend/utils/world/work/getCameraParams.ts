  ...
  if (bp) {
    ...
    objects.forEach((obj) => {
      ...
      if (obj instanceof PerspectiveCamera) {
        sectionKey = objNames[3] as WorkWorldSectionKey; // Sec1, Sec2, Sec3
        index = objNames[4]; // 0, 1, ...
        // オフセットの場合: Cam_BP_XX_Offset_SecN_0
      } else if (obj instanceof Object3D && objNames[3] === 'Offset') {
        sectionKey = objNames[4] as WorkWorldSectionKey; // Sec1, Sec2, Sec3
        index = objNames[5]; // 0, 1, ...
      }
